...
contract PangolinRouter is IPangolinRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'CootieRouter: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'CootieRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = PangolinLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'CootieRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'CootieRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'CootieRouter INSUFFICIENT_B_AMOUNT');
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'CootieRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'CootieRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PangolinLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    {
        require(path[0] == WAVAX, 'CootieRouter: INVALID_PATH');
    ...
    function swapTokensForExactAVAX(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WAVAX, 'CootieRouter: INVALID_PATH');
        amounts = PangolinLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'CootieRouter EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PangolinLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WAVAX, 'CootieRouter INVALID_PATH');
        amounts = PangolinLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'CootieRouter EXCESSIVE_INPUT_AMOUNT');
        IWAVAX(WAVAX).deposit{value: amounts[0]}();
        assert(IWAVAX(WAVAX).transfer(PangolinLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust AVAX, if any
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'CootieRouter INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'CootieRouter INSUFFICIENT_OUTPUT_AMOUNT');
